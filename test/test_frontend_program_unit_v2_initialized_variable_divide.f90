program test_frontend_program_unit_v2_initialized_variable_divide
    ...
    write (*, '(a)') 'frontend program-unit-v2 initialized variable-divide checks: ok'
...
contains
    ...
    subroutine assert_accepted(initializer)
        ...
        character(len=*), parameter :: source_hash = 'initialized-variable-divide-input'
        ...
        call frontend_parse_program_unit_v2('initialized-variable-divide.f90', trim(source), &
        ...
        if (.not. ok .or. unit%execution_part%sequence%assignment_count /= 2_int64 .or. &
            ...
            error stop 'valid initialized variable divide AST changed'
        end if
        ...
        call assert_span(unit%root%span%file, unit%root%span%source_hash, &
            ...
            'initialized-variable-divide.f90', source_hash, 0_int64, &
        ...
        call assert_provenance(unit%variable%span%file, unit%variable%span%source_hash, &
            ...
            'initialized-variable-divide.f90', source_hash)
        call assert_span(unit%execution_part%sequence%assignment(1)%span%file, &
            ...
            'initialized-variable-divide.f90', source_hash, first_start, first_end)
        call assert_span(unit%execution_part%sequence%assignment(2)%span%file, &
            ...
            unit%execution_part%sequence%assignment(2)%span%end_byte, 'divide', &
            'initialized-variable-divide.f90', source_hash, second_start, second_end)
        call assert_span(unit%execution_part%print%span%file, &
            ...
            'print', 'initialized-variable-divide.f90', source_hash, print_start, print_end)
        ...
        if (.not. ok .or. index(trim(serialized), '(assignment-count 2)') == 0 .or. &
            ...
            error stop 'initialized variable divide SX serialization changed'
        end if
    ...
    subroutine assert_rejected(initializer, assignment, printed_name)
        ...
        source = 'program main'//new_line('a')//'  integer :: x'//new_line('a')// &
            ...
            new_line('a')//'  print *, '//trim(printed_name)//new_line('a')// &
            'end program main'//new_line('a')
        call frontend_parse_program_unit_v2('initialized-variable-divide-negative.f90', &
            trim(source), 'initialized-variable-divide-negative-input', unit, ok, message)
        if (ok) error stop 'invalid initialized variable divide was accepted'
    ...
    subroutine assert_provenance(file_name, source_hash, start_byte, end_byte, label, &
        ...
        if (trim(file_name) /= trim(expected_file) .or. trim(source_hash) /= trim(expected_hash) .or. &
            ...
            error stop 'initialized variable divide provenance changed: '//trim(label)
        end if
    ...
    subroutine assert_span(file_name, source_hash, start_byte, end_byte, label, &
        ...
        if (trim(file_name) /= trim(expected_file) .or. trim(source_hash) /= trim(expected_hash) .or. &
            start_byte < 0_int64 .or. end_byte <= start_byte .or. &
            (expected_start >= 0_int64 .and. start_byte /= expected_start) .or. &
            (expected_end >= 0_int64 .and. end_byte /= expected_end)) then
            error stop 'initialized variable divide provenance changed: '//trim(label)
        end if
    end subroutine assert_span

end program test_frontend_program_unit_v2_initialized_variable_divide
