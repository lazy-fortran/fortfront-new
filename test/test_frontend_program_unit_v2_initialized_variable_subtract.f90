program test_frontend_program_unit_v2_initialized_variable_subtract
...
contains
    ...
    subroutine assert_accepted(initializer)
        ...
        character(len=*), parameter :: source_file = 'initialized-variable-subtract.f90'
        character(len=*), parameter :: source_hash = 'initialized-variable-subtract-input'
        ...
        call frontend_parse_program_unit_v2(source_file, trim(source), source_hash, unit, ok, message)
        if (.not. ok .or. unit%execution_part%sequence%assignment_count /= 2_int64 .or. &
            unit%execution_part%print_count /= 1_int64 .or. trim(unit%root%name) /= 'main' .or. &
            trim(unit%variable%name) /= 'x' .or. &
            ...
            error stop 'valid initialized variable subtract changed'
        end if

        first_start = int(index(trim(source), '  x = '//trim(initializer)) - 1, int64)
        first_end = first_start + int(len_trim(initializer) + 5, int64)
        second_start = int(index(trim(source), '  x = x - x') - 1, int64)
        second_end = second_start + int(len('  x = x - x') - 1, int64)
        ...
        call assert_span(unit%root%span%file, unit%root%span%source_hash, &
            unit%root%span%start_byte, unit%root%span%end_byte, 'root', source_file, source_hash, &
            0_int64, int(len_trim(source) - 1, int64))
        ...
        call assert_provenance(unit%variable%span%file, unit%variable%span%source_hash, &
            unit%variable%span%start_byte, unit%variable%span%end_byte, 'variable', source_file, source_hash)
        call assert_span(unit%execution_part%sequence%assignment(1)%span%file, &
            ...
            unit%execution_part%sequence%assignment(1)%span%end_byte, 'initializer', source_file, source_hash, &
        ...
        call assert_span(unit%execution_part%sequence%assignment(2)%span%file, &
            ...
            unit%execution_part%sequence%assignment(2)%span%end_byte, 'subtract', source_file, source_hash, &
            second_start, second_end)
        call assert_span(unit%execution_part%print%span%file, unit%execution_part%print%span%source_hash, &
            unit%execution_part%print%span%start_byte, unit%execution_part%print%span%end_byte, &
            'print', source_file, source_hash, print_start, print_end)
        ...
        if (.not. ok .or. index(trim(serialized), '(assignment-count 2)') == 0 .or. &
            ...
            error stop 'initialized variable subtract serialization changed'
        end if
    ...
    subroutine assert_rejected(assignment, printed_name)
        ...
        call frontend_parse_program_unit_v2('initialized-variable-subtract-negative.f90', trim(source), &
            'initialized-variable-subtract-negative-input', unit, ok, message)
        if (ok) error stop 'invalid initialized variable subtract was accepted'
    ...
    subroutine assert_provenance(file_name, source_hash, start_byte, end_byte, label, &
        ...
        if (trim(file_name) /= trim(expected_file) .or. trim(source_hash) /= trim(expected_hash) .or. &
            ...
            error stop 'initialized variable subtract provenance changed: '//trim(label)
        end if
...
end program test_frontend_program_unit_v2_initialized_variable_subtract
