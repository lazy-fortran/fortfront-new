program test_frontend_program_unit_v2_initialized_multiplier
    ...
    write (*, '(a)') 'frontend program-unit-v2 initialized multiplier checks: ok'

contains

    subroutine assert_accepted(initializer, multiplier)
        character(len=*), intent(in) :: initializer, multiplier
        character(len=1024) :: source
        character(len=*), parameter :: source_hash = 'initialized-multiplier-input'
        ...
        source = 'program main'//new_line('a')//'  integer :: x'//new_line('a')// &
            '  x = '//trim(initializer)//new_line('a')//'  x = x * '//trim(multiplier)// &
            new_line('a')//'  print *, x'//new_line('a')//'end program main'//new_line('a')
        call frontend_parse_program_unit_v2('initialized-multiplier.f90', trim(source), &
        ...
        if (.not. ok .or. unit%execution_part%sequence%assignment_count /= 2_int64 .or. &
            ...
            error stop 'valid initialized multiplier changed'
        end if

        first_start = int(index(trim(source), '  x = '//trim(initializer)) - 1, int64)
        first_end = first_start + int(len_trim(initializer) + 5, int64)
        second_start = int(index(trim(source), '  x = x * '//trim(multiplier)) - 1, int64)
        second_end = second_start + int(len('  x = x * ') + len_trim(multiplier) - 1, int64)
        ...
        call assert_span(unit%root%span%file, unit%root%span%source_hash, &
            ...
            'root', 'initialized-multiplier.f90', source_hash, 0_int64, &
        ...
        call assert_span(unit%declaration%span%file, unit%declaration%span%source_hash, &
            ...
            'declaration', 'initialized-multiplier.f90', source_hash, 0_int64, &
        ...
        call assert_provenance(unit%variable%span%file, unit%variable%span%source_hash, &
            ...
            'initialized-multiplier.f90', source_hash)
        call assert_span(unit%execution_part%sequence%assignment(1)%span%file, &
            ...
            'initializer', 'initialized-multiplier.f90', source_hash, first_start, first_end)
        call assert_span(unit%execution_part%sequence%assignment(2)%span%file, &
            ...
            'multiplier', 'initialized-multiplier.f90', source_hash, second_start, second_end)
        call assert_span(unit%execution_part%print%span%file, &
            ...
            'print', 'initialized-multiplier.f90', source_hash, print_start, print_end)
        ...
        if (.not. ok .or. index(trim(serialized), '(assignment-count 2)') == 0 .or. &
            ...
            index(trim(serialized), '(right-operand '//trim(multiplier)//')') == 0 .or. &
            index(trim(serialized), '(source-hash '//source_hash//')') == 0) then
            error stop 'initialized multiplier serialization changed'
        end if
    ...
    subroutine assert_rejected(initializer, assignment)
        ...
        source = 'program main'//new_line('a')//'  integer :: x'//new_line('a')// &
            ...
            new_line('a')//'  print *, x'//new_line('a')//'end program main'//new_line('a')
        call frontend_parse_program_unit_v2('initialized-multiplier-negative.f90', trim(source), &
            'initialized-multiplier-negative-input', unit, ok, message)
        if (ok) error stop 'invalid initialized multiplier was accepted'
    ...
    subroutine assert_provenance(file_name, source_hash, start_byte, end_byte, label, &
        ...
        if (trim(file_name) /= trim(expected_file) .or. trim(source_hash) /= trim(expected_hash) .or. &
            ...
            error stop 'initialized multiplier provenance changed: '//trim(label)
        end if
...
end program test_frontend_program_unit_v2_initialized_multiplier
